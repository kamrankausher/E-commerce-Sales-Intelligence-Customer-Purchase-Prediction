...
import sqlite3
import pytest
from app.database.connection import get_connection, run_query
...
def test_run_query_with_params():
    ...
    res = run_query("SELECT ? AS test_val", (5,))
    assert len(res) == 1
    assert res[0]["test_val"] == 5
...
def test_parse_sql_file_exists():
    ...
    queries = AnalyticsService.parse_sql_file(filepath)
...
def test_parse_sql_file_not_found():
    ...
    queries = AnalyticsService.parse_sql_file("fake_path_does_not_exist.sql")
    assert queries == []

def test_get_query_success():
    """Test retrieving a query successfully by index."""
    query = AnalyticsService.get_query(0)
    assert "SELECT" in query.upper()

def test_get_query_out_of_bounds():
    """Test retrieving a query with an out of bounds index raises ValueError."""
    with pytest.raises(ValueError):
        AnalyticsService.get_query(999)
